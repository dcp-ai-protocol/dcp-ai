...
export function verifyV2SignedBundle(
  ...
  // Optional DCP-05–09 artifact validation (non-breaking: only validate if present)
  const extArtifacts = [
    ["schemas/v2/commissioning_certificate.schema.json", signedBundle.commissioning_certificate, "commissioning_certificate"],
    ["schemas/v2/vitality_report.schema.json", signedBundle.vitality_report, "vitality_report"],
    ["schemas/v2/decommissioning_record.schema.json", signedBundle.decommissioning_record, "decommissioning_record"],
    ["schemas/v2/digital_testament.schema.json", signedBundle.digital_testament, "digital_testament"],
    ["schemas/v2/succession_record.schema.json", signedBundle.succession_record, "succession_record"],
    ["schemas/v2/memory_transfer_manifest.schema.json", signedBundle.memory_transfer_manifest, "memory_transfer_manifest"],
    ["schemas/v2/dispute_record.schema.json", signedBundle.dispute_record, "dispute_record"],
    ["schemas/v2/rights_declaration.schema.json", signedBundle.rights_declaration, "rights_declaration"],
    ["schemas/v2/delegation_mandate.schema.json", signedBundle.delegation_mandate, "delegation_mandate"],
  ];
  for (const [schemaPath, artifact, name] of extArtifacts) {
    if (artifact != null) {
      const result = validateOne(schemaPath, artifact, baseDir);
      if (!result.valid) {
        result.errors.forEach((e) => errors.push(`${name}: ${e}`));
      }
    }
  }
  if (errors.filter((e) => !e.startsWith("V2 WARNING")).length > 0) {
    return { verified: false, errors };
  }
